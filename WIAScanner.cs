...
namespace ServerScan
{
    class WIAScanner
    {
        ...
        public static List<Bitmap> Scan(String scannerId, ScanSettings settings)
        {
            ...
            foreach (DeviceInfo info in manager.DeviceInfos)
            {
                if (info.DeviceID == scannerId)
                {
                    ...
                    {
                        throw new Exception("Không thể kết nối đến máy quét. Vui lòng kiểm tra lại.");
                    }
            ...
            if (device == null)
            {
                throw new Exception("Không tìm thấy máy quét.");
            }
        ...
        public static Hashtable GetDevices()
        {
            ...
            {
                throw new Exception("Không thể khởi tạo trình quản lý máy quét.\nVui lòng kiểm tra file wiaaut.dll có tồn tại trong thư mục system32 và đã được kích hoạt (Mở run và chạy 'regsvr32 wiaaut.dll').");
            }
